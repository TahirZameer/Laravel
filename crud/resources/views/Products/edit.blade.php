        ...
        <div class="row d-flex justify-content-center">
            <div class="col-md-10">
                <div class="card border-1 shadow-lg my-4">
                    ...
                    <form enctype="multipart/form-data" action="{{ route('Products.update',$product->product_id) }}" method="post">
                        @method('put')
                        @csrf
                        <div class="card-body">
                            <div class="mb-3">
                                <label for="product_name" class="form-label h5">Product Name</label>
                                <input type="text" value="{{ old('product_name', $product->product_name) }}" class="@error('name') is-invalid @enderror form-control form-control-lg" placeholder="Name" name="product_name"/>
                                @error('product_name')
                                    <!-- {{ $message }} -->
                            ...
                            <div class="mb-3">
                                ...
                                <input type="Integer" value="{{ old('price', $product->price) }}" class="@error('price') is-invalid @enderror form-control form-control-lg" placeholder="price" name="price"/>
                                @error('price')
                                    <!-- {{ $message }} -->
                            ...
                            <div class="mb-3">
                                ...
                                <textarea placeholder="Description..." type="text" class="form-control form-control-lg text-black" rows="4" cols="50" name="description"> {{ old('description', $product->product_description)}} </textarea>
                            </div>
                            <div class="mb-3">
                                ...
                                @if (!empty($product->product_image) && file_exists(public_path('storage/uploads/products/' . $product->product_image)))
                                        <img class="w-50 my-3" src="{{ asset('storage/uploads/products/' . $product->product_image) }}" alt="">
                                @endif
